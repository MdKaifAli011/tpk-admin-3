...
/** Sample vars for previewing default templates in admin. */
export function getDefaultTemplateSampleVars(key) {
  const base = config?.siteUrl || "";
  const sample = {
    name: "Alex",
    form_name: "Contact form",
    reset_url: base ? `${base}/reset-password?token=sample` : "",
    thread_title: "How do I prepare for the test?",
    thread_slug: "how-do-i-prepare",
    is_approved: true,
    test_name: "Practice Test 1",
    percentage: 85,
    correct_count: 17,
    total_questions: 20,
    view_url: base ? `${base}/results/sample` : "",
  };
  return sample;
}
 ...
 * Exported for admin "view default" API.
 */
export function getDefaultContent(key, vars = {}) {
  switch (key) {
