...
api.interceptors.request.use(
  (config) => {
    // When sending FormData (file uploads), do not set Content-Type so the browser
    // sends multipart/form-data with the correct boundary. Next.js request.formData()
    // requires Content-Type to be multipart/form-data or application/x-www-form-urlencoded.
    ...
    if (typeof window !== "undefined") {
      ...
      // For discussion routes:
      // STRICT SEPARATION: If we are on the student side (Main App), we should NOT send the admin token
      // unless we are explicitly in an Admin context (which usually uses a different layout/api instance, but here it's shared).
      // However, to fix the user's issue: "Checking without login (main) side then post thread approved" -> This means
      // they want to act as GUEST even if Admin token exists.

      let token = adminToken; // Default to admin token logic for general usage

      if (isStudentApi) {
        // If it's a student API, prefer student token.
        // If NO student token exists, DO NOT fall back to admin token effectively making them a "Guest".
        // EXCEPT if the specific request might be from the Admin Panel using the same API lib?
        // The Admin Panel is under /admin. The Main App is under /.
        // We can check window.location to see if we are in Admin Panel.

        const currentPath = window.location.pathname;
        const isAdminPanel = currentPath.includes("/admin");

        if (isAdminPanel) {
          // We are in Admin Panel, so yes, use Admin Token for moderation
          token = adminToken;
        } else {
          // We are in Main App (Student Side)
          // Only use student_token. If it's missing, token is null -> Guest.
